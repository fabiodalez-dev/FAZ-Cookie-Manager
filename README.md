...
### 1.21.2 — 2026-06-30
- **Fixed**: map tiles and lazy-loaded embeds now blocked before consent (#163, #167). Map widgets (Leaflet/OpenStreetMap, Bricks Map) load their tiles as runtime `<img>`, and Bricks' native lazy-load swaps an embed URL into `iframe.src` at runtime — both bypassed the blocker, so the resource loaded (and the visitor's IP reached the third party) with consent denied. The `src` setter is now gated on the image and iframe prototypes: a cross-origin resource matching a blocked provider in a denied category is parked (no request) until consent, then restored. Fast-pathed so same-origin/relative/`data:`/`blob:` resources — theme assets and uploads — are untouched. Verified live on a Bricks sandbox.
