...
- **Compatibility**: the long-standing `[faz_cookie_policy]` shortcode (with `site_name` / `contact` / `show_table` attributes and the "How to Manage Cookies" section) is **unchanged and still supported**. The new variant is opt-in via the `_complete` suffix — chosen for human readability over the more terse `_v2` so the migration path reads naturally for operators.
