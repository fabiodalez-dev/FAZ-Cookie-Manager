...
- **Added**: inline age-appropriate consent gate (GDPR Art. 8) — an optional age-confirmation checkbox that gates only the accept path, never Reject/withdraw/close, keeping button weight equal. Self-declared affirmation only, not a substitute for Art. 8(2) parental-consent verification. Default off.
...
- **Fixed**: a banner cached under a previous site address kept requesting assets from the old origin (#195) — an address change now drops the cache, and a render-time repair rewrites and persists a stale origin, covering a restored database that never fires the hook.
- **Fixed**: the setup wizard's scan reported a fraction of the cookies the Cookies page found. The wizard called a server-side endpoint that reads `Set-Cookie` headers, and PHP does not execute JavaScript — so analytics, advertising and embedded-player cookies were invisible to it. The browser-based engine the Cookies page already used (hidden iframes, so it sees what a visitor receives) is now shared by both, with real per-page progress in the wizard.
- **Fixed**: a blocked Cookie Policy save now names the offending field, opens its section and focuses it, instead of doing nothing; background scans under a web SAPI run through WP-Cron with honest counts; third-country transfer labels resolve in the banner/policy language rather than the ambient locale.
