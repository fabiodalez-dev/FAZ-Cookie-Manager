...
* **Guided setup wizard (NEW in 1.25.0)** -- a first-run wizard detects your environment (multilingual plugin, page cache, WooCommerce, existing consent data) and configures defaults that match your jurisdiction, explaining each choice in plain language. It pre-selects the more protective option and never touches consent expiry or equal-weight buttons. Existing sites are treated as already set up and are never nagged.
