...
* **FlyingPress** — the plugin purges (and re-warms) FlyingPress's page cache automatically whenever a banner, cookie, category or setting is saved, so a change never keeps serving stale banner markup. FlyingPress does not honor `DONOTCACHEPAGE`, so the plugin also hooks its documented `flying_press_is_cacheable` filter to skip caching on country-dependent pages, matching the bypass every other supported cache gets. The consent scripts are excluded automatically from minification and from "Delay all JavaScript": FlyingPress 4 uses its delay/defer filters, while FlyingPress 5 receives the same keywords in its in-memory delay-exclusion config without changing your saved FlyingPress settings. You normally do not need to touch anything. If a future FlyingPress release changes those internals the automatic v5 exclusion degrades quietly (with a note in the debug log when `WP_DEBUG` is on); if you ever notice the banner appearing only after the first click, add `faz-cookie-manager` to FlyingPress's "Delay JavaScript" exclusion keywords as a fallback.
