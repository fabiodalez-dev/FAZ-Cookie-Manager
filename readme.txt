...
* Added: cookie duration and description are now shown in the visitor's language, not only the category labels (#214). A bundled catalogue supplies wording for known cookies and translates simple retention periods such as "2 years". It is only consulted when nothing is stored for that language, so a description you wrote yourself is never replaced, and an unrecognised or free-form duration keeps the value you entered. Applies to the banner, the [faz_cookie_table] shortcode and the generated Cookie Policy; no database change is involved.
