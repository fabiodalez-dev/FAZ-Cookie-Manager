...
class Cookie_Policy_Shortcode {
	...
	public function render( $atts = array() ) {
		...
		$site_name  = sanitize_text_field( $atts['site_name'] );
...
' );
		...
		<p><?php
			...
			echo esc_html( sprintf( __( 'Cookies are small text files that are placed on your device when you visit %s. They are widely used to make websites work more efficiently, as well as to provide information to the website owners.', 'faz-cookie-manager' ), $site_name ) );
		?></p>

		<h2><?php esc_html_e( 'How We Use Cookies', 'faz-cookie-manager' ); ?></h2>
		<p><?php
			/* translators: %s: site name */
			echo esc_html( sprintf( __( '%s uses cookies for several purposes, including:', 'faz-cookie-manager' ), $site_name ) );
		?></p>
		<ul>
