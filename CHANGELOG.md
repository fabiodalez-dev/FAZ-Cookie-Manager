...
- **73 more known trackers in the blocking database and one-click catalogue.** Expanded `Known_Providers` (the engine behind server-side and client-side blocking, cookie shredding and scanner enrichment) and the admin blocker-template catalogue with widely-used services not previously covered:
  - *Ad-tech / SSP / DMP:* AdRoll, The Trade Desk, Xandr, PubMatic, Magnite/Rubicon, OpenX, Media.net, LiveRamp, Comscore, Nielsen, Yahoo, Adform, Teads, Sharethrough, Index Exchange, Equativ/Smart AdServer, GumGum, TripleLift, Yieldmo, ID5, Lotame, BidSwitch, 33Across, Tealium.
  - *B2B intent / lead intelligence:* 6sense, Demandbase, Bombora, Leadfeeder, ZoomInfo.
  - *Analytics & A/B testing:* Chartbeat, Parse.ly, StatCounter, Inspectlet, Simple Analytics, Piwik PRO, Dynamic Yield, AB Tasty, Monetate, Kameleoon.
  - *Marketing automation / email:* Brevo/Sendinblue, Constant Contact, GetResponse, Braze, Iterable, AWeber, Omnisend, MailerLite, Campaign Monitor, SendGrid.
  - *Chat / support:* Help Scout, Gorgias, ManyChat.
  - *Reviews / social proof:* Yotpo, Bazaarvoice, REVIEWS.io, Judge.me, Loox.
  - *Video:* Brightcove, JW Player, Vidyard, Kaltura, Flowplayer.
  - *Push notifications:* OneSignal, PushEngage, iZooto.
  - *Surveys / forms:* SurveyMonkey, Jotform, Qualtrics.
  - *Web fonts:* Font Awesome, Monotype (fonts.com).
  - *Consent managers:* iubenda, CookieYes, Didomi.

  Every entry uses host- or path-scoped patterns (no bare CDN domains) so the runtime `src`/`href` gate and network interceptors cannot park a legitimate cross-origin asset. Categories follow existing conventions (consent tools and fonts → functional, video → marketing, ad-tech/DMP → marketing, session/analytics → analytics).
