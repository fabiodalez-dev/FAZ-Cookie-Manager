...
### Fixed
- **PayPal (and Square, Braintree, Klarna) payment SDK blocked on non-WooCommerce sites — reported on the #125 thread.** The payment-gateway auto-whitelist exempted the PayPal SDK only on WooCommerce cart/checkout pages, so a site using a non-WooCommerce payment plugin (Forminator, Paid Memberships Pro, Easy Digital Downloads, Give, …) had `paypal.com/sdk/js` blocked by the consent script blocker. `window.paypal` never got defined and the payment form's own script then threw `paypal is not defined`, so the PayPal button never rendered. The checkout SDK loaders (`paypal.com/sdk/js`, `paypalobjects.com/api/checkout.js`, `squareup.com`, `braintreegateway.com`, `x.klarnacdn.net`) are now always allowed site-wide — exactly like Stripe already was — because they are necessary for the transaction the visitor initiated, wherever the payment form renders. The PayPal *marketing* pixel (`paypal.com/tagmanager/pptm.js`) is deliberately not exempted and stays blocked until consent; WooCommerce gateway-plugin script handles remain scoped to checkout/cart.
- **`[faz_cookie_policy_complete]` text missing from POT/PO catalogues.** The GDPR, CCPA and LGPD Markdown scaffolds now expose all 33 policy sections through gettext, so site owners can translate or adjust the generated policy with the normal WordPress translation workflow. The Czech PO/MO ships those sections fully translated from the reviewed Czech templates. Runtime overrides are accepted only for the matching WordPress locale and only when every required `{{PLACEHOLDER}}` is preserved; otherwise the renderer keeps the bundled localized section. Gettext policy edits now also change `data-faz-policy-version`.
