...
### Changed
- **Geo-Targeting enables live jurisdiction enforcement.** When enabled, the 47-law catalogue drives pre-consent category defaults, blocking, banner-law selection, mandatory Do-Not-Sell/GPC/revisit controls, independent sensitive-processing opt-in and Consent Mode defaults. The `faz_geo_ruleset_runtime` filter remains an explicit developer override. Cache Compatibility Mode is ignored while enforcement is active so one country's consent model cannot be cached for another.
