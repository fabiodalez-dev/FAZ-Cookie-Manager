...
- **Wizard completion now verifies persistence.** Banner, Google Consent Mode and main settings writes are read back before success is returned; a silent database write failure can no longer mark onboarding complete or redirect as though the requested configuration existed.
