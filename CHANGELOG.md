...
### Added
- **POPIA (South Africa) jurisdiction.** South Africa joins the supported privacy regimes: a POPIA law choice in the setup wizard, a ZA geo region, and a full set of Cookie Policy templates in every bundled language. The templates follow POPIA's opt-in model — s.11(1)(a) consent, the Information Officer as the accountable contact, data-subject rights under s.23–25, objection under s.11(3), and the automated-decision safeguards of s.71 — and scope the 30-day response period to PAIA s.25 access requests, where it actually applies, rather than to every request.
