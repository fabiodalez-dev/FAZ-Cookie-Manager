...
- **29 more known trackers in the blocking database and one-click catalogue.** Expanded `Known_Providers` (the engine behind server-side and client-side blocking, cookie shredding and scanner enrichment) and the admin blocker-template catalogue with widely-used services not previously covered: ad-tech / DMPs (AdRoll, The Trade Desk, Xandr, PubMatic, Magnite/Rubicon, OpenX, Media.net, LiveRamp, Comscore, Nielsen, Yahoo), analytics (Chartbeat, Parse.ly, StatCounter, Inspectlet, Simple Analytics, Piwik PRO), A/B testing (Dynamic Yield, AB Tasty), marketing automation (Brevo/Sendinblue, Constant Contact, GetResponse), video (Brightcove, JW Player), push/reviews (OneSignal, Yotpo) and consent managers (iubenda, CookieYes, Didomi). Every entry uses host- or path-scoped patterns (no bare CDN domains) so the runtime `src`/`href` gate cannot park a legitimate cross-origin asset.
