...
### Fixed
- Consent records were refused, silently, on any site whose page cache holds HTML for more than a day (#292). The origin token that proves a consent POST came from a page this site rendered was accepted for the current 12-hour bucket and the previous one only, so a cached page stopped being able to record consent 12 to 24 hours after it was stored — while LiteSpeed Cache ships a 604800-second (7 day) public TTL and WP Rocket and W3TC defaults are measured in days. Nothing looked broken: the banner worked, the visitor saw a normal confirmation, the POST is fire-and-forget, and the only thing missing was the Art. 7(1) accountability record. The reporter measured it on two production sites — 57 consents logged and 286 refused in one day, the share climbing with the age of the cached HTML and dropping after a purge. A token is now accepted for seven days by default, `faz_consent_token_max_age` moves that window for a site that caches for longer (clamped to one bucket at the bottom and 30 days at the top), and the token is minted by the same class that accepts it instead of by a second copy of the arithmetic in the frontend. Widening the window does not weaken what the token proves: it is keyed to this installation's salts, so it cannot be minted elsewhere at any age, and a browser cannot replay it from another site, because the same-origin check — unchanged here — reads headers a page script is forbidden to set. A client that is not a browser sets those headers itself, so that check was never what stops one: the per-IP and per-consent-id throttles are, both unchanged, and the refusal accounting is now bounded by a throttle of its own. A refused record is no longer silent either — it is counted, reported in System Status and written to the error log once an hour.
- The refused-records figure reported a fraction of the truth from a thousand upwards. The count was printed with an integer conversion but handed the locale-grouped string, which stops at the thousands separator: 2,002 refused records rendered as "2" — on the one row whose job is to say how many records were lost, and in every language, since every locale groups thousands somehow. The reporting site's own volume, 286 refusals a day, reaches four figures inside a week. The row's own tests never caught it because both fixtures were under a thousand.
- The refusal tally was a fixed window anchored at its first refusal, not the rolling seven days it claimed. On a site that kept refusing, the figure collapsed to one the instant that anchor aged out and climbed again — a sawtooth reported as "the last 7 days": three hundred refusals a day showed 2,100 on day six and 299 on day seven. It is now seven per-day counters summed on read, so the claim is true, and it keeps the property the anchored shape got right: a cause that stopped over a week ago reads as zero without waiting for another refusal to roll it over. A tally written by the first cut of this feature is still reported after the upgrade.
- Four of the five ways a consent record can be discarded were counted nowhere, so "no row on System Status" could not be read as "nothing was lost". A request refused for want of a same-origin signal is reachable by a real browser whenever the page's origin differs from the WordPress Address — a scheme or port mismatch, a sandboxed iframe — and such a site loses every record while the page reports nothing; a rate-limited request is answered 200 and dropped, which visitors sharing one outbound address can hit; and a failed database write is always a real visitor's real consent, behind a 500 the fire-and-forget client never sees. All five are now counted under their own cause and reported separately, because folding automated traffic into the same figure as records a cache lost would make an alarming number out of noise. The row appears with a zero state, and names the most recent occurrence, so a resolved problem can be told from a live one.
- The refusal accounting no longer offers an unauthenticated caller a guaranteed database write per request. The tally was written before the endpoint's own per-IP throttle, and the only gate in front of it reads request headers a non-browser client sets freely, so a bogus token bought a write on the one route that must stay reachable without authentication. The write is now rate-limited per visitor per cause, on its own key so a junk token cannot spend the budget a real visitor needs moments later. The case the tally exists for — many distinct visitors each refused once from stale cached HTML — is unaffected; a flood from one address is counted once per window, which the row states.
- The pageview endpoint carried the same defect as #292, unfixed, in a second copy of the same arithmetic (#296): it accepted only the current 12-hour bucket and the previous one, so pageview, banner-view and banner-choice events posted from cached HTML older than about a day were refused with a 403 and the Dashboard's counters under-reported in silence. It now uses the same seven-day window, with its own `faz_pageview_token_max_age` filter, and mints its token in the class that accepts it.
- The System Status "Copy status" report put one `label: value` line per row, which held only while every cell was written on a single source line. `textContent` returns the template's own newlines and tabs — a browser collapses whitespace when it paints, not when it reads — so the refused-records cell produced a ragged, tab-indented continuation line with no label in front of it. Separately, `<br>` contributes no text at all, so the Active Plugins list came out as one run-together string ("Akismet 5.3WooCommerce 9.1…") in the snapshot people paste into bug reports. Nor did the builder see a boundary the markup made: the per-cause breakdown is echoed straight after the "Most recent: … ago." sentence, with nothing between them, so the copied text read "ago.Stale origin token" — the browser breaks a line at a block element, `textContent` does not. All of it is fixed in the report builder rather than in the cells, by one rule that marks every block boundary before the whitespace is collapsed, so rows added later inherit the format.
- The "Copy status" report dropped half of the one table on that page that is not a label/value pair. The blocked Set-Cookie table has four data columns under a real header row, and the report read the first two cells of every row — so the cookie name and its category were copied, while Request Path and Blocked At, the two columns an administrator checking that Set-Cookie blocking is not breaking checkout or login would have pasted the report for, were not. The shape is decided per row now: a pair is still `label: value`, and a row with more columns keeps all of them under the table's own column names.
- A consent POST carrying no origin token at all was refused by WordPress rather than by this plugin, and so counted nowhere. The route declared the token as a required argument, and WordPress rejects a missing required argument before the callback runs — keyed on the parameter being absent, so an empty token reached the handler and a missing one did not. The cause existed, System Status named that exact shape in its copy, and it was the one shape the code counting it never saw. The endpoint now answers such a request itself, with the same 403 and the `missing_token` code the empty case already returned, and records it. Automated traffic is what normally reaches this, which is why it is reported under its own cause rather than folded in with records a cache lost.
- `.faz-help` was only ever styled inside a form group, so the class rendered at full body size and colour everywhere else — the new row's explanation, and three pre-existing help paragraphs, sat indistinguishable from the primary text they were meant to sit beneath.
...
### Added
- Language-aware cookie policy publication at the end of guided setup. The shortcode pins the selected language and jurisdiction; repeated submissions reuse the existing page, and custom banner links are preserved. The option is off by default and says, before it is ticked, that the page is published straight from the template and that the company details must be filled in. It is disabled for a language that has no policy template, and a page that cannot be created or linked no longer stops setup: the jurisdiction, banner and consent logging are saved first, and the page problem is reported as a warning.
- Published-page autocomplete in banner and Cookie Policy link fields, with keyboard navigation, stale-response protection and manual URL entry. Simplified the wizard layout for desktop and mobile.
- Optional weekly cookie-definition downloads, disabled by default. Settings names the remote host; disabling the option cancels the schedule, and the callback rechecks permission. Failed downloads preserve existing definitions and are reported on the Cookies screen (#258).
- Advanced Consent Mode help now explains that analytics_storage follows Analytics even when Google tags are moved to Performance or a custom category (#260).
- The consent log now records whether the server would have served each GPC exception, so inconsistent markers can be identified after the fact (#285). A GPC exception says a visitor clicked Accept on a blocked embed; the consent cookie cannot prove that, because a script on the site's own pages can write the same values, and no token can prove it either — anything issued into the page is readable by a script in that page. So the record does not claim the click was genuine. It claims consistency: the server marks an exception as served only when the GPC signal was present, no Do Not Sell request was in force, the cookie really carried the grant and its marker, the page had actually shown a placeholder for that service, and that service's category is one the signal closes. Exceptions the visitor is simply carrying from page to page are marked as carried rather than judged again, and a carried exception keeps the verdict it started with: one the server could not corroborate stays unverified on every later page. When the site's Referrer-Policy cuts the Referer down to the bare origin, the page is taken from the consent request instead, so a genuine click on an inner page is not judged against the home page. The consent log shows one pill per service with a tooltip, explains the three states beside the filter — "unverified" means the server could not corroborate the circumstances, not that the exception is proven forged — and can be filtered down to the rows that carry an exception, whatever their status.
- System Status lists the catalogued services that are blocked without leaving any visible trace (#279): they set no cookies, so they appear in no cookie declaration, and when they are blocked on a script or stylesheet pattern no placeholder is shown either. Services that render an embed placeholder when blocked are not listed. Before consent a stylesheet or script from one of them simply does not load — fonts fall back, or a control appears not to work — and the symptom points at the theme, the cache or the CDN instead of here. Google Fonts, the reported case, stays blocked: German courts have held that loading it from Google without consent is unlawful, and serving those fonts from your own server avoids both the block and the question.

### Fixed
- Elementor Video widgets are judged on the embed URL the browser actually loads (`youtube.com/embed`, `youtube-nocookie.com/embed` in privacy mode, `player.vimeo.com/video`), then on the URL typed in the widget. A rule or exemption written for a plain iframe therefore applies to a widget holding a `youtube.com/watch`, `youtu.be` or `vimeo.com` link too.
- Bricks Google Maps widgets now display a consent placeholder, preserve their options until consent, and reveal Google Maps in per-service preferences without requiring a fabricated cookie entry. The widget is judged as the API script Bricks enqueues for it, so saved blocking rules, exemptions (including one on the `bricks-google-maps` handle) and a per-service choice for Google Maps apply to it as well. Delayed Bricks initialisation is retried when its script loads after Google's callback, once, and never for a map Bricks has already drawn.
- WP Rocket Delay JavaScript now excludes FAZ inline bootstrap markers, the consent logger's included: delayed, its listener was not attached yet when the visitor made the first choice, and that decision never reached the consent log. The runtime also merges an already loaded static configuration before making blocking decisions, preventing a delayed inline merge from leaving the provider and cookie maps absent.
- WooCommerce attribution replay is decided once, when the first restored script finishes after Sourcebuster exists. If WooCommerce's own script has not run yet — WP Rocket delaying it, say — FAZ leaves it alone: WooCommerce adds `setOrderTracking` to the object it localised earlier, finds Sourcebuster and initialises by itself, and a replay on top of it would have counted a second page view in `sbjs_session`.
- GPC audit carry-forward preserves an unknown legacy verdict, requires the current consent cookie's grant and exception marker, and stops at any intervening decision without the exception. Consent list and CSV export share the same exception filter.
- Embed inventory observations use an atomic upsert, avoiding duplicate-key errors for concurrent same-second renders while preserving the first observation. Verified with MySQL and WordPress's SQLite translator.
- Malformed cookie-definition refreshes preserve the last usable dataset; malformed legacy rows cannot crash lookups. Automatic refresh timestamps retain the absolute instant across daylight-saving changes.
...
- Sourcebuster is now gated on Marketing instead of Analytics, the category WooCommerce itself checks through the WP Consent API. Before, a visitor who accepted only Marketing never got attribution, and one who accepted only Analytics loaded a library WooCommerce then refused to start. New scans classify the `sbjs_*` cookies as Marketing too.
- Sites that had already scanned the `sbjs_*` cookies kept them as Analytics in their own cookie list, and that list is what FAZ consults first. With the script now gated on Marketing, a visitor who accepted only Marketing got the cookies written by WooCommerce and deleted by FAZ within a fraction of a second, so the order still lost its source. The upgrade moves those rows to Marketing — only the rows a scan wrote and nobody saved since, the rule the Functional sale/sharing migration uses. A category an administrator saved is kept, and so is an imported one, since an import carries a category without saying whether a person chose it. A one-time notice says what was moved and asks to check the category of any row it left.

- On sites with plain permalinks or query-string language URLs, every consent-log row recorded the home page, because the page URL was stored without its query string. The routing parameters that identify a page (`?p=`, `?page_id=`, `?lang=` and the like) are now kept; credentials, fragments and every other parameter are still dropped.
- GPC inventory refreshes now track the latest persisted snapshot, distinguish plain WordPress permalinks, and only replace records after a completed HTML blocking pass. The dashboard reports when consent logging itself is disabled.
...
### Known limitation
- The exception record shows consistency, not authenticity: a forgery that reproduces a real click's circumstances stays indistinguishable. Preventing that is not available to a plugin of this kind — a script running on the site's own pages has the visitor's own powers, so no token issued into the page can prove a human click — so the limit is stated rather than papered over (#285).
...
- Installs created before 1.17.2 still had the Functional category flagged as sale and sharing, the schema default of the time. GPC is enforced under every applicable law and opts the visitor out of every flagged category, so on those sites maps, videos and other functional embeds never loaded for GPC visitors, whatever they clicked. The upgrade clears the legacy flags under the two conditions described below and notifies the administrator when a row changes.
- The migration also reaches the installs it was written for. Where a site exposes no Do Not Sell link the Sale / Sharing column was HIDDEN until this release, and `share_personal_data` was added to the schema later by a column default that stamped every existing row without touching its dates — so there the flags are cleared regardless of dates. This can also reset an imported classification because imports have no reliable provenance; the administrator is notified and can restore an intentional classification. Where the column was visible, the date rule stands.
- When the migration changes that row, the administrator is told: a dismissible notice explains that Functional is no longer treated as sale/sharing, what it means for visitors sending Global Privacy Control, and where to set it again.
- The consent log distinguishes a record a privacy signal created, with the banner still unanswered, from a visitor's partial save: it carries `meta.signal_only`, shown as its own audit pill.
...
- With GPC or a Do Not Sell request, the banner appeared on the first page only; from the second page visitors saw just the revisit icon, as if they had chosen. The signal creates a consent record on the first page — it holds the binding opt-out — and the init path read that record as the visitor's decision. The record is now marked `undecided` until the visitor answers the banner. Under an opt-in law (GDPR) the banner keeps being offered on every page until they do; under an opt-out law (CCPA/CPRA) the signal is itself the decision and the banner is not re-offered, and neither is it after a Do Not Sell request, which is an explicit act under either law. Any choice clears the mark. The AMP bridge and the Paid Memberships Pro integration treat an undecided record as no choice, and so does the TCF API (`useractioncomplete`, Purpose 1).
- The same missing reload made every page view of a GPC or Do Not Sell visitor re-apply the opt-out as if it were new: `fazcookie_consent_update` fired on each page, sending a consent-log request, a Consent Mode update and a TCF `useractioncomplete` every time. The signal markers are now restored while the signal is asserted, so the opt-out is reported once.
- Withdrawing that exception now works from the preference centre. GPC forces the category to denied, so an unticked service matched its category, no denial was written, and the save put the grant straight back — Reject All was the only way out.
- Only a click on that service's own blocked-content card creates the exception. `window._fazAcceptService()` is a public function any script on the page can call, which could otherwise exempt a service from the visitor's opt-out permanently and log it as the visitor's own act.
- The server enforces the same rule as the browser: under a binding signal, a `svc.<id>:yes` without its marker no longer reopens a closed category on the PHP render path, where the tag would run before any script could clear it.
- A pruned marker is written back to the cookie. Deleting it from the store alone left it in the cookie, so a rejection could be logged as carrying a GPC exception, and a marker could outlive the signal that justified it.
- Under an opt-out law (CCPA/CPRA) the banner is not re-offered after a signal: there the signal is the consumer's decision, and 11 CCR §7026(k) asks a business to wait twelve months before inviting an opted-out consumer to opt back in. A Do Not Sell request is not re-offered under either law — the visitor filled in a form. Only GPC, which the browser sends on its own, leaves the rest of the banner's question unanswered.
- A click parked by the age gate keeps its origin: after the visitor ticks the age box the replayed grant is still the embed click it was, so the exception is minted instead of being dropped by the binding pass.
- AMP: a standing Do Not Sell request now binds the AMP endpoints as GPC does — an AMP "Accept" could re-grant the categories it had opted out of — and the per-cookie choices inside an excepted service travel with it, so a refusal of one cookie is no longer dropped into a grant.
...
### Known limitation
- The GPC exception marker records consent state; it does not authenticate a human click against hostile JavaScript already executing on the page (issue #285). A token accessible to that same script alone would not provide that guarantee.

### Added
- A consent liveness matrix: 32 configurations (applicable law, GPC, Do Not Sell, per-service consent, sale/share flag on the embed's category) through four scenarios, each checked on the same page and after a reload, against one written rule — a consent the visitor gives stays alive, and dies only where a binding opt-out requires it. An end-to-end spec reproduces the reported configuration on the real stack, and a second unit suite runs the real init path across GPC, Do Not Sell and no signal under both laws.
...
### Added
- Footer consent links via `[faz_cookie_settings type="link"]`, with theme styling, custom labels and the existing delegated preference-center handler. The default shortcode remains a button. Banner settings document both formats and the floating-widget toggle.
- System Status explains host-managed WordPress cron and provides a site-specific cPanel command without requiring WP-CLI.
- Button corner radius is configurable in Banner → Colours (issue #191). The banner buttons already read `--faz-btn-border-radius`, which nothing ever set, so the only way to change it was a CSS snippet; the field now emits it. Empty keeps the shipped 2px, so existing banners render identically. The value is shape-checked on save because it is written into a CSS custom-property declaration.
...
- A browser scan can only see the site through a logged-in administrator's session, so it cannot tell whether a cookie it observed also reaches visitors (issue #243). The observations it sets aside are now kept with the domain and lifetime the scan measured, and offered on the Cookies screen with a one-click declaration, instead of asking for them to be retyped by hand. Declaring lifts the name-based rule that keeps infrastructure cookies such as `_lscache_vary` out of the banner and the policy; it never makes them deletable, and WordPress authentication cookies can never be declared at all. Every catalogue is complete in all six shipped languages.
### Changed
...
- **A preference center opened from `[faz_cookie_settings]` could silently discard saved IAB vendor choices.** The revisit widget refreshes the vendor switches from the stored consent before the panel is seen; the shortcode handler did not, so the panel showed the server-rendered defaults. Because the save path reads those switches, "Save my preferences" then wrote the defaults back over the visitor's real vendor decisions. The shortcode now performs the same refresh, which is what that function's contract ("when reopening the preference center") always required. Only affects sites with IAB TCF enabled (#253).
...
- **The page-cache veto no longer fires where no country can be resolved.** Enforcement being on does not make output vary by country on an install with no Cloudflare header, no server GeoIP module and no GeoLite2 database: every visitor gets the same most-protective ruleset, so the page is invariant and the veto cost the publisher their whole page cache for nothing. With enforcement now on by default that would have reached most shared hosting — recreating, by default, the very reports this release answers. Gated on a country source being configured, overridable with `faz_has_country_signal_source`.
- **`ip-api.com` is no longer declared as an external service.** The readme described sending the visitor's IP to it for country resolution; no such call exists anywhere in the plugin, and the detection chain is entirely local (trusted CDN header, server GeoIP module/extension, self-hosted GeoLite2). The disclosure and two stale docblocks now match the code.
### Removed
...
- **Cookie duration and description follow the visitor's language** (#214). A banner offered in two languages previously translated only the category labels, leaving each cookie's duration and description in whatever language they were entered in, so the declaration read as a mix. A bundled catalogue now supplies descriptions for known cookies — including prefix entries such as `_ga_`, which cover the concrete names generated at runtime — and translates simple retention periods like "2 years". The rule is not "only when empty" — that would leave the problem in place, because scanners and imports write the same English string into every language slot, so the Italian column holds "2 years" and is not empty at all. The catalogue also replaces a value it RECOGNISES as its own English text; anything else is left exactly as entered. So an administrator's own wording always wins, a free-form or ambiguous duration ("1 year or longer") is returned verbatim because the parser refuses it, an already-translated value is untouched, and legacy rows gain translations on read with no database rewrite. Wired into the three surfaces that render cookie inventory: the banner declaration, the `[faz_cookie_table]` shortcode and the Cookie Policy renderer.
- **The bundled cookie wording can be overridden or switched off per site.** `faz_cookie_content_i18n_description` and `faz_cookie_content_i18n_duration` receive the resolved text along with the cookie slug and the requested language, so a site can correct an entry, supply wording for a cookie the catalogue does not know, or return an empty string to fall back to whatever is stored. Bundled content is a reviewed starting point rather than a verdict, and a cookie declaration is a legal statement about a specific site — so the operator needs a way to have the last word that does not involve editing files the next update overwrites.
- **Consent state is now available to site integrations on every page.** `fazcookie_consent_ready` announces the initial and updated state, `getFazConsent().services` exposes per-service decisions, and `getFazCookieConsent()` resolves a declared cookie using the same precedence as the blocker. Late-loading WP Consent API and Microsoft UET/Clarity bridges replay the most recent state instead of missing a one-shot event.
...
- **Frontend and backend hot paths do less repeated work.** Generated configuration/CSS assets are content-addressed and written atomically; provider matching is memoized; settings, banner and GCM reads avoid repeated option walks; scanner imports invalidate caches once per batch; retention jobs and CSV exports work in bounded batches/streams instead of loading unbounded result sets.
- **The Czech catalogue was resynchronised to the current source catalogue.** The compiled translation now matches the committed PO, including restored quote-split strings and the footer heading.

### Fixed
- **reCAPTCHA renders again on sites installed before 1.17.2.** A working reCAPTCHA needs two hosts — the API endpoint on `www.google.com` and the widget script on `www.gstatic.com` — but only the first was ever seeded, so those installs allowed the endpoint and blocked the script it loads. A one-time migration adds the missing pattern, and only where the site already whitelists the reCAPTCHA endpoint; a site that never used reCAPTCHA, or removed the entry, is left alone and nothing is ever removed.
...
- **Granular consent no longer deletes the cookies of an accepted category.** With per-service or per-cookie consent enabled, a cookie was kept only if it carried an explicit allow token, so a visitor who accepted analytics had the analytics cookies removed from the browser anyway. The documented precedence — explicit per-cookie, then explicit per-service, then category consent — is restored: with no explicit decision the category now decides, which also stops the first cleanup pass from wiping necessary cookies. An explicit refusal still overrides an accepted category. Reachable only when granular consent is switched on; both toggles default to off, and the defect over-deleted rather than under-deleted, so no data was ever exposed.
- **Retention cleanup never ran on SQLite-backed installs.** The batched purge relied on a MySQL-only `DELETE … LIMIT`, which SQLite rejects; every batch failed and the run reported the same zero it reports when there is nothing to delete. Consent logs and pageviews therefore grew without bound on those sites while the retention window appeared to be working — a data-minimisation failure, not a performance one. Both purges now page through primary keys and delete by key, and a failed query is distinguished from an empty result instead of being read as success.
- **The script-blocking whitelist is no longer re-seeded over an administrator's own edits.** The seeder's only guard was "the list is currently empty", which cannot tell a fresh install from a site that deliberately cleared the whitelist, so every upgrade that advanced the migration version wrote the defaults back. It now runs once behind a marker, like its sibling seeders, and no longer back-fills entries broader than the shipped defaults — a whitelist grants permission, and quietly widening it weakens the site it touches.
- **Tracking resources inside `<noscript>` are now gated independently.** A mixed block can no longer inherit the first provider's category or consent decision, and non-tracking fallback markup remains untouched.
...
### Added
- **Administrator-editable Cookie Policy sections, isolated by jurisdiction and language.** The Cookie Policy screen now exposes each effective scaffold section as an optional textarea. Shipped wording is the placeholder rather than a copied value, so leaving a section empty keeps it upgradeable; authored Markdown retains the normal `{{PLACEHOLDER}}` substitution pipeline. Languages without a bundled scaffold, including Slovak, can use the jurisdiction fallback as a starting point and store their own text under that language. Every override carries its original section heading as a drift anchor: after a scaffold update, a mismatch disables the override and falls back to reviewed wording instead of placing legal text under the wrong heading.
- **POPIA (South Africa) jurisdiction.** South Africa joins the supported privacy regimes: a POPIA law choice in the setup wizard, a ZA geo region, and a full set of Cookie Policy templates in every bundled language. The wizard ships a conservative consent-based preset under s.11(1)(a), while explicitly noting that POPIA also permits the s.11(1)(b)-(f) justifications and that sites relying on them need a separate assessment. The templates cover the Information Officer as the accountable contact, data-subject rights under s.23–25, objection under s.11(3), and the automated-decision safeguards of s.71, and scope the 30-day response period to PAIA s.25 access requests, where it actually applies, rather than to every request.
- **Guided first-run setup wizard (8 steps).** A new install is walked through jurisdiction-correct defaults instead of being left on a generic configuration: the wizard detects the environment (multilingual plugin, page cache, WooCommerce, pre-existing consent data), explains each choice in plain language, and pre-selects the more protective option. First setup—or switching between opt-in and opt-out models—applies the canonical expiry and notice controls shown in review; reopening without changing consent model preserves custom expiry and button visibility. Upgrading installs are treated as already onboarded and are never nagged; the wizard is armed only on genuine fresh installs, with a dismissible re-entry card on the Dashboard.
- **A/B testing of banner variants.** Run two or more of your existing banners simultaneously with a persistent random split (sticky cookie) and read the accept rate per variant from the consent log on the Dashboard, so consent UX can be optimised with evidence. Only *active* banners take part, each independently compliant — the feature cannot author a dark pattern. The accept rate is reported as accepted/decisions with a documented denominator and a per-status breakdown, so a CCPA variant's rate is not diluted by Do-Not-Sell opt-outs. Default off; skipped under Cache Compatibility Mode so cached HTML stays visitor-invariant.
- **Inline age-appropriate consent gate (GDPR Art. 8).** An optional age-confirmation checkbox rendered directly above the Accept/Reject buttons. It gates only the accept path — Reject, withdraw and close are never gated, and Accept is never disabled or greyed while Reject stays active, so button weight stays equal (EDPB 03/2022). An un-affirmed accept shows an inline message, moves focus to the checkbox, announces politely, and writes no consent. This is a self-declared affirmation and is **not** a substitute for the parental-consent verification Art. 8(2) requires. Default off.
- **Schrems II third-country transfer disclosure, per cookie.** An optional per-cookie disclosure that a service transfers personal data to a country without an EU adequacy decision, surfaced both in the preference-center cookie declaration and in the generated Cookie Policy, so a site can obtain the informed consent Art. 44–49 requires. The wording is deliberately neutral: it names the fact and the safeguard the admin describes, and never asserts that the safeguard is legally sufficient. WordPress-internal and necessary-admin cookies can never surface, and the disclosure is invariant across consent state so Cache Compatibility Mode is unaffected. Default off.
- **Banner resilience against ad-block cosmetic filter lists.** An optional single deferred re-assert that keeps the legally required consent notice visible when a cosmetic filter list (EasyList Cookie and similar) hides elements whose class or id contains "cookie"/"consent". One check, no loop, no MutationObserver, no cookie wall; it never touches consent or script blocking, and a later dismissal still hides the banner. This is resilience of a mandatory notice, not circumvention of a privacy tool. Default off.
- **Placeholder blocking for Smash Balloon Instagram Feed and the Elementor Video widget** — contributed by [@roboes](https://github.com/roboes) (#190).

### Added
- **The generated Cookie Policy text is now editable.** The shipped Markdown scaffolds are reviewed starting points, but they cannot describe every operator's actual processing — and the plugin only ships templates for eight languages, which left users needing a policy in a ninth with no option but to edit plugin files that the next update overwrites. A collapsed **Policy text** card on the Cookie Policy screen now offers one box per section, per jurisdiction and per language. Filling one replaces that section; leaving it empty keeps the reviewed text, so a future update can still improve wording nobody deliberately changed. Placeholders such as `{{COMPANY_NAME}}` keep resolving inside custom text, which passes through exactly the same escaping as shipped text. A language the plugin ships no template for can be selected too: the bundled scaffold acts as the editing baseline while your text is stored against the language you picked. Each override records the section heading it was written against — if a later release reorders or rewords the templates, the override deactivates and the reviewed original is shown, because for a legal document reverting to the shipped text is the only acceptable failure and putting the operator's wording under the wrong heading is not.

### Changed
- **A `jurisdiction="…"` shortcode override no longer bypasses that jurisdiction's mandatory fields.** `[faz_cookie_policy_complete jurisdiction="popia-southafrica"]` previously rendered even when the fields that regime requires were unset — the reasoning being that a degraded policy beat a blank page. For a legal document that trade is the wrong way round: POPIA section 18 requires the responsible party's identity and address, and a policy that silently omits them reads as complete while failing the requirement it exists to meet. The mandatory-field check now runs against whichever jurisdiction is actually being rendered. An administrator sees the configuration notice explaining what is missing; anonymous visitors receive nothing at all, so an incomplete policy is never published. **If you use a shortcode override, fill in that jurisdiction's required fields on the Cookie Policy screen.**

### Security
- **The consent dashboard widget is now gated on capability**, so its aggregate consent figures are no longer readable by any logged-in user who can see the Dashboard.
- **The CCPA opt-out endpoints now carry a strict same-origin check.** The Do-Not-Sell / Do-Not-Share mutations validate Fetch Metadata headers with a Referer fallback, closing a cross-site request path that could flip a visitor's opt-out state.

### Fixed
- **Provider scripts whose blocking pattern ends on a separator were never blocked — the HubSpot tracker ran before consent (#196).** The URL matcher demanded a separator character *after* the match to rule out an accidental substring hit, but a pattern that already ends on `/` or `.` carries its own right-hand boundary: `js.hs-scripts.com/` cannot be a prefix of a longer domain label because the slash has closed it. Requiring another separator meant such a pattern could only ever match at the very end of a URL, so an ordinary `js.hs-scripts.com/12345.js` was left unblocked — the next character being a digit. Twenty of the shipped provider definitions carry at least one pattern in this shape. Fixed identically in the PHP and JavaScript matchers.
- **A banner cached under a previous site address kept requesting assets from the old origin (#195).** The rendered banner is cached with absolute URLs to the plugin's own assets, so moving a site — a local build restored on a server, staging promoted to production — left the cache asking the previous host for the revisit icon, which browsers surface as a private-network access prompt when that host was localhost. Two independent defences now cover it: a change of `siteurl`/`home` drops the cache, and a render-time repair rewrites a stale asset origin and persists the fix, which also covers a restored database that rewrites the address row directly and never fires the hook.
- **The setup wizard's scan reported a fraction of the cookies the Cookies page found.** The wizard called a server-side endpoint that fetches pages with `wp_remote_get()` and reads their `Set-Cookie` headers. PHP does not execute JavaScript, so everything written by analytics, advertising and embedded players was invisible to it — on a typical site that is nearly everything, and the step could report a single cookie where the real scanner found dozens. It was not a shorter scan but a different instrument. The browser-based engine that the Cookies page has always used — which loads pages in hidden iframes and therefore observes what a visitor actually receives — is now shared by both surfaces, so they cannot disagree again. Public paths discovered on a different home/admin hostname are retried through the exact admin origin when that alias can serve them; if no page is observable, the scan stops with actionable framing/origin diagnostics instead of importing a misleading server-only result. The wizard's progress bar is consequently real (pages completed, cookies and scripts seen so far) rather than an animation, and the step now says to keep the tab open, because unlike the old server-side crawl this one runs in the browser.
- **Wizard completion is now atomic and non-destructive on re-entry.** Banner, Google Consent Mode and main settings writes are read back and committed together; any failure rolls all three back instead of leaving a partial setup. Reopening the same consent model preserves custom expiry/button choices, while an actual opt-in/opt-out switch reapplies the reviewed canonical preset. Fresh installs use the site locale (not the administrator profile), untouched geo regions follow the selected jurisdiction, and false-like REST values can no longer enable switches accidentally.
- **A blocked Cookie Policy save now explains itself instead of doing nothing.** Validation failures write a persistent error naming the offending field, open the collapsed section containing it, and move focus there.
- **Background scans under a web SAPI now run through WP-Cron and report honest counts**, instead of appearing to complete work the request had no time to do.
- **Third-country transfer labels resolve in the banner/policy language**, not the ambient request locale, so on a multilingual site they no longer diverge from the country and safeguard values beside them.
...
- **Cookie Policy generator routed through the WordPress gettext pipeline.** The generated cookie-policy sections now flow through the standard translation pipeline (`_x()` with per-section context), so the policy honours the site locale and any `.mo` overrides, falling back to the bundled Markdown when a translation is missing.

### Changed
- **Payment gateway scripts: per-gateway opt-in instead of an automatic allow-list (reported on the #125 thread).** A Forminator PayPal form was throwing `paypal is not defined` because the consent blocker was blocking `paypal.com/sdk/js`; the gateway auto-whitelist only covered WooCommerce checkout/cart pages, so it never applied on a non-WooCommerce payment form. The fix does **not** auto-allow the SDK — a payment SDK can set cookies and fingerprint, so loading it before consent is a GDPR/ePrivacy decision, not something the plugin should do silently. Instead, **Settings → Script Blocking → Payment gateways** now offers a clearly-labelled, per-gateway toggle (PayPal, Stripe, Square, Braintree, Klarna, Mollie, Amazon Pay). When a store owner enables a gateway, its *payment* scripts (never its marketing pixel — PayPal's `pptm.js` stays blocked) are allowed before consent site-wide, for payment forms that live outside a WooCommerce checkout (Forminator, Paid Memberships Pro, Easy Digital Downloads, Give, …). All off by default; a genuine WooCommerce checkout/cart page is still exempt automatically as strictly necessary. The cookie shredder follows the same rule, so a gateway's cookies are exempted exactly when its scripts are. **Migration:** the Stripe SDK was previously loaded before consent on every page; if you use Stripe elements outside a WooCommerce checkout (express buttons on product/account pages), enable Stripe under Settings → Script Blocking → Payment gateways after updating.

- **Cookie shredder now runs on `template_redirect`.** The server-side non-consented-cookie shredder moved from `send_headers` to `template_redirect` (priority 1), where the main query is resolved so the WooCommerce checkout/cart conditionals and page exclusions are reliable, while still running before any template output so the deletion headers stay valid. An explicit per-service / per-cookie denial now also wins over the admin cookie whitelist on both the server and the client, so a deliberately-withdrawn cookie is always removed.

### Fixed
- **Category toggles rendering as editable text when another plugin filters `wp_kses_allowed_html` — issue #188.** `faz_allowed_html()` merged its own `<input>` allow-list as the *first* argument to `array_merge()`, so any active plugin that hooks `wp_kses_allowed_html` and declares its own `input` entry overwrote that key and dropped `type` — `wp_kses()` then stripped `type="checkbox"` from the "Customize consent preferences" toggles, which defaulted to `type="text"` and rendered as editable fields. The allow-list is now built the other way round, merging the plugin's required `<input>` attributes into whatever the filter returns, so both sides survive regardless of filter order.
- **Per-service toggle invisible for JS-injected embeds on block-first sites (#134/#146).** When an embed's `iframe.src` was assigned from JavaScript, the iframe src-gate parked the URL and blocked the embed before it entered the DOM, so the block-time MutationObserver — the only place the per-service reveal fired — saw an empty `src` and skipped it. The embed was blocked but the visitor never got its granular consent toggle. The src-gate now surfaces the per-service toggle when it parks a blocked embed.
- **Consent banner double-initialising under Cloudflare Rocket Loader — issue #185.** Rocket Loader can execute the plugin's init script twice; `_fazInit()` is now guarded against double-execution so the banner does not bind its handlers or render twice.
- **Notice dismiss link unlabelled for screen readers.** The custom icon-only dismiss link on the payment-gateway and WooCommerce admin notices had no accessible name; both now carry a translatable `aria-label` and mark the decorative glyph `aria-hidden`.
