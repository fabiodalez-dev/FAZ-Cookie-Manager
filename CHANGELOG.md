...
### Fixed
- Elementor watch/share URLs are resolved to their player URL before matching categories, so the existing YouTube `/embed` template rules also work on Video widgets.
- Bricks Google Maps widgets now display a consent placeholder, preserve their options until consent, and reveal Google Maps in per-service preferences without requiring a fabricated cookie entry. Saved blocking rules and exemptions apply to the widget as well as the API script. Delayed Bricks initialisation is retried when its script loads after Google's callback.
- WP Rocket Delay JavaScript now excludes FAZ inline bootstrap markers. The runtime also merges an already loaded static configuration before making blocking decisions, preventing a delayed inline merge from leaving the provider and cookie maps absent.
- WooCommerce attribution replay is limited to the instance present before Sourcebuster was restored, preventing duplicate initialisation when WooCommerce itself loads later.
