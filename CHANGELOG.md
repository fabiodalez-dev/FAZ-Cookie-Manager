...
- **The whitelist seeder no longer undoes the 1.17.2 blocking fix.** It carried a second copy of the default list, and the copy had drifted: sites landing on it were seeded with Google Fonts, Maps, OAuth and generic CDNs allowed before consent — the entries 1.17.2 deliberately removed. The seeder now reads the shipped defaults instead of repeating them.
