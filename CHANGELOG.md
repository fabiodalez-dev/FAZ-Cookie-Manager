...
- The pageview endpoint carried the same defect as #292, unfixed, in a second copy of the same arithmetic: it accepted only the current 12-hour bucket and the previous one, so pageview, banner-view and banner-choice events posted from cached HTML older than about a day were refused with a 403 and the Dashboard's counters under-reported in silence. It now uses the same seven-day window, with its own `faz_pageview_token_max_age` filter, and mints its token in the class that accepts it.
