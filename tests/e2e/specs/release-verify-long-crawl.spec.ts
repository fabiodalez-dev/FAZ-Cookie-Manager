 ...
 * jobs and their settle checkpoints run — while route fulfilment models the
 * fully page-cached case where those pages never boot PHP. Import therefore
 * succeeds only if the heartbeat emitted by the real engine keeps the real
 * server session alive for the entire crawl.
 */
...
function seedLongCrawlPagesAndSnapshot(): string {
  return wpEval(`
    ...
    for ( $i = 0; $i < ${SEEDED_PAGE_COUNT}; ++$i ) {
      $slug = sprintf( 'faz-long-crawl-%03d', $i );
      $wpdb->insert(
        $wpdb->posts,
        array(
          'post_author'           => 1,
