...
ds_ok( false !== strpos( $controller, "const MISSED_SCANS_THRESHOLD = 2;" ), 'a cookie must be missing from more than one scan before deletion is offered' );
ds_ok( false !== strpos( $controller, 'public function record_scan_observations' ), 'the tally is updated from the scan result' );
ds_ok( false !== strpos( $controller, 'if ( ! $is_complete ) {' ), 'an incomplete scan cannot add to the tally — its silence is not evidence' );
ds_ok( false !== strpos( $controller, 'continue; // Hand-added cookies are never judged by a scan.' ), 'a hand-added cookie is never judged by a scan' );
ds_ok( false !== strpos( $controller, 'continue; // Seen again: the tally resets by omission.' ), 'seeing a cookie again clears its tally' );
ds_ok( false !== strpos( $controller, 'public function deletable_stale_keys' ), 'the threshold is applied server-side, not left to the browser' );

echo "== The scan must declare its own completeness ==\n";
ds_ok( false !== strpos( $scan_api, "empty( \$metrics['incremental'] ) && empty( \$metrics['earlyStopReason'] )" ), 'incremental runs and early stops do not count as complete' );
ds_ok( false !== strpos( $scan_api, "0 === \$pages_scanned\n\t\t\t? false" ), 'a scan that visited nothing is never complete' );
ds_ok( false !== strpos( $scan_api, "\$result['deletable_stale_keys']" ), 'the import reports which entries have earned deletability' );
