...
namespace FazCookie\Frontend {
	 ...
	 * floor; an absent value becomes 182. CCPA carries a 365-day floor so an
	 * opt-out cannot expire before the statutory re-ask interval, plus the
	 * 3650-day ceiling advertised by the banner UI.
	 */
	class Frontend {
...
namespace FazCookie\Admin\Modules\Cookies\Includes {
	...
	class Cookie_Categories {
		...
		public function get_prior_consent() { return ! empty( $this->item->prior_consent ); }
		public function get_sell_personal_data() { return ! empty( $this->item->sell_personal_data ); }
		public function get_share_personal_data() { return ! empty( $this->item->share_personal_data ); }
	}
}
...
namespace {
	...
	Category_Controller::$items = array(
		...
		array( 'slug' => 'marketing', 'name' => 'Marketing', 'visibility' => 1, 'sell_personal_data' => 1 ),
	...
	// denied category when the visitor returns to a classic page.
	$GLOBALS['faz_test_cookie'] = str_replace( 'marketing:no', 'marketing:yes', $GLOBALS['faz_test_cookie'] ) . ',svc.ads:yes,svc.maps:yes';
	$_COOKIE['fazcookie-consent'] = $GLOBALS['faz_test_cookie'];
	$_SERVER = array( 'HTTP_AMP_SAME_ORIGIN' => 'true', 'HTTP_SEC_GPC' => '1' );
	$gpc_check = $bridge->handle_check( new Faz_AMP_Test_Request( $base + array( 'consentStateValue' => 'accepted' ) ) );
	amp_same( $gpc_check->get_data()['purposeConsents'], array( 'analytics' => true, 'marketing' => false ), 'GPC overrides a prior AMP sale/share grant while preserving analytics' );
	$gpc_update = $bridge->handle_update( new Faz_AMP_Test_Request(
		$base + array(
			'consentStateValue' => 'accepted',
	...
	amp_same( $gpc_update->get_data()['purposeConsents'], array( 'analytics' => true, 'marketing' => false ), 'GPC also overrides a new AMP grant on the same request' );
	$gpc_cookie = faz_parse_consent_cookie( $GLOBALS['faz_test_cookie'] );
	amp_same( isset( $gpc_cookie['gpc'] ) ? $gpc_cookie['gpc'] : '', '1', 'AMP persists the GPC audit marker' );
	amp_same( isset( $gpc_cookie['marketing'] ) ? $gpc_cookie['marketing'] : '', 'no', 'AMP persists the sale/share category as denied under GPC' );
