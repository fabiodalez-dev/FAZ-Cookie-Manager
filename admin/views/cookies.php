	...
	<div class="faz-card" style="margin-top:16px;">
		...
		<div class="faz-card-body">
			<div class="faz-help" style="margin:0 0 12px;">
				<?php esc_html_e( 'Add custom URL patterns to block scripts or iframes until the visitor consents to the specified category. Each pattern is matched against script/iframe src, inline code, and enqueued handle names. The plugin already blocks 147+ known services automatically.', 'faz-cookie-manager' ); ?>
			</div>
			...
			<button class="faz-btn faz-btn-secondary" id="faz-add-rule" type="button"><?php esc_html_e( '+ Add Rule', 'faz-cookie-manager' ); ?></button>
		</div>
	</div>
	...
	<div class="faz-card" style="margin-top:16px;">
		...
		<div class="faz-card-body">
			...
			<div style="display:flex;align-items:center;gap:8px;margin:12px 0;">
				...
				<button class="faz-btn faz-btn-outline faz-btn-sm" id="faz-copy-shortcode" type="button" data-copied-msg="<?php echo esc_attr__( 'Shortcode copied!', 'faz-cookie-manager' ); ?>"><?php esc_html_e( 'Copy', 'faz-cookie-manager' ); ?></button>
			</div>
			<details style="margin-top:8px;">
				...
				<div style="margin-top:8px;font-size:13px;line-height:1.6;">
					...
					<table class="faz-table" style="font-size:13px;">
						...
						<tbody>
							<tr>
								...
								<td><?php esc_html_e( 'Comma-separated list of columns. Available: name, domain, duration, description, category', 'faz-cookie-manager' ); ?></td>
							</tr>
							<tr>
								<td><code>category</code></td>
								<td><em><?php esc_html_e( '(all)', 'faz-cookie-manager' ); ?></em></td>
								<td><?php esc_html_e( 'Filter by category slug (e.g. analytics) or ID', 'faz-cookie-manager' ); ?></td>
							</tr>
							<tr>
					...
					<p style="margin-top:8px;"><strong><?php esc_html_e( 'Example:', 'faz-cookie-manager' ); ?></strong> <code>[faz_cookie_table columns="name,duration,description" category="analytics"]</code></p>
					<p style="margin-top:4px;"><?php esc_html_e( 'The legacy shortcode [cookie_audit] is also supported for backward compatibility.', 'faz-cookie-manager' ); ?></p>
				</div>
			</details>
		</div>
	</div>
...
document.getElementById('faz-copy-shortcode').addEventListener('click', function() {
	...
	if (navigator.clipboard) {
		navigator.clipboard.writeText(text).then(function() {
			FAZ.notify(document.getElementById('faz-copy-shortcode').dataset.copiedMsg);
		});
	} else {
