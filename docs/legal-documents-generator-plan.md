...
3. **A consent-invalidation mechanism already exists.** `invalidate_consents()` in `admin/modules/settings/api/class-api.php` (~line 998) proves the established revision-bump mechanics. The policy decision uses those mechanics inside one recoverable Cookie Policy endpoint, rather than asking the browser to coordinate two non-atomic REST writes.
...
1. **Ledger** `faz_legal_doc_acknowledged` (§4.1): per document, the last hash the admin acknowledged. Hash tracked = the document's *default* render combination (saved jurisdiction + saved/default language) — the same combination the version meta already exposes. Tracking all j×l combos would multiply prompts without changing the decision.
   ...
   **Tracking the default combination alone is not enough, so it is not what we do.** Overrides are keyed `[jurisdiction][lang][index]`; an operator who rewrites only the French text, or only a non-default jurisdiction, changes that variant's `data-faz-policy-version` and under a default-only ledger would get **no notice at all** — the published document changes and nobody is asked. The ledger therefore folds **the default combination plus every jurisdiction × language pair that has a saved override** into one stable review token. That set is bounded by what the operator actually authored (usually zero or one or two entries), not by the 32-cell j×l grid. Any tracked variant whose hash moves raises the notice; the current UI presents the aggregate previous/current token rather than claiming to identify a single changed variant. §6 covers a non-default variant edit raising the notice while its default render hash stays unchanged.
2. **Detection**: on Legal Documents / Cookie Policy admin page load (admin-side only — zero frontend cost), compute current hash via the existing `policy_version_hash()` path; when it differs from the ledger, show a notice: *"Your Privacy Policy has changed since you last confirmed it (version a1b2c3 → d4e5f6)."* with three actions:
   ...
   - **"Material change — re-ask consent"** → calls `cookie-policy/acknowledge-material-version`, which persists a recovery intent, bumps `general.consent_revision` using the established settings mechanics, verifies the stored value, then records the review token. Existing frontend mechanics in `script.js` re-show the banner to returning visitors. A confirm dialog states the consequence (all returning visitors re-prompted).
...
**Files in the shipped Cookie Policy slice:** `cookie-policy-generator/includes/class-version-ledger.php`, notice rendering in `admin/views/cookie-policy.php`, the nonce-protected POST routes `cookie-policy/acknowledge-version` and `cookie-policy/acknowledge-material-version`, and `admin/assets/js/pages/cookie-policy.js`. The later shared-document UI can move this contract under `legal-documents`; no frontend files change because the consent-revision plumbing already exists end-to-end.
