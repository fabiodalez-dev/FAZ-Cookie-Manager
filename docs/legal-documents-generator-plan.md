...
| `Renderer` | Extract the pipeline body of `render()` into `render_for( Document_Config $doc, array $atts )`. `render( $atts )` becomes a two-liner: `return self::render_for( Document_Registry::get( 'cookie-policy' ), $atts );`. Settings option, data builder, disclaimer default, wrapper class, filter tag all read from `$doc`. `build_cookie_list_html()`, `collect_transfer_disclosures()`, `build_services_list()` stay where they are and become the cookie-policy `data_builder`'s private helpers. | `Renderer::render()` output byte-identical (golden-file test) |
...
- The existing **Cookie Policy page stays exactly as it is** in this programme. Deliberate: it is a shipped, hardened editor; folding it into a tabbed mega-page in the same release that introduces three new documents doubles the regression surface for pure aesthetics. The Legal Documents page lists all documents including Cookie Policy, with Cookie Policy linking to its existing page. Nav consolidation is a candidate for a later cosmetic release, explicitly out of scope here.
...
- Every new option is registered in `uninstall.php` under the existing `remove_data_on_uninstall` gate.
