...
- one banner focus-loop test: pre-existing `fixme` for issue #62, not executed in this run and not claimed as covered by it. It has since been investigated and fixed rather than left deferred: `_fazLoopFocus()` was called only from the preference-centre path, so the trap was never armed when the banner itself rendered and a visitor who never opened preferences could Tab out of a `role="dialog"`. The call is now made when the banner becomes visible; the test is re-enabled and passes, and removing the call fails it on `toBeFocused()`;
...
The content of PRs #267–#274 is present in full: each source head's changed files were compared against this branch and none is missing, with the only differences being deliberate supersession (#270's partial catalogues by #272's complete ones), the merged changelog, and files touched by more than one PR. None of those eight heads is a git ancestor of this branch, however — the work was re-applied rather than merged. GitHub will therefore not auto-close the source PRs, and merging one of them after this branch would re-apply its commits or conflict rather than fast-forward; they have accordingly been closed as superseded. Four admin JavaScript files that were CRLF on #273 (dashboard.js, consent-logs.js, languages.js, gcm.js) are LF here: the re-application normalised their line endings, which changes no behaviour but makes those files read as whole-file rewrites in the diff. CI Quality, three consent browsers, PHP/WordPress checks and the available security scans pass on the final code. The original checkout’s six uncommitted policy edits were preserved outside this isolated consolidation; each was later confirmed byte-identical to its counterpart here and the stale copies discarded.
